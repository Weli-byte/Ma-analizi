...
6. Team ratings (Elo) updated only after result; predictions use pre-match rating.
7. Final test set never used for selection/tuning. Ensemble stacking only on OOF.
8. Post-kickoff data never enters a pre-match snapshot.
9. Leakage audit (S2 harness, S10 audit command) is a release gate.
